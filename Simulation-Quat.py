...
wi=np.array([0.0,0.0,0.0]) # Vitesse angulaire initiale
...
q=np.zeros(simuNPoints, dtype=np.quaternion)
...
for i in range(1,simuNPoints):
    ...
    if position[2]<0:
        ...
        trajecto[i]=position
        # Calcul de rotation
        q[i],wi=updateRotation(selfAcceleration,q[i-1],wi,h)
    # Nouveau pas
    time+=h
